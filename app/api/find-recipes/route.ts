...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to parse AI response', aiResponse: recipesString }, { status: 500 })
    }

    if (!Array.isArray(recipes) || recipes.length !== 3) {
      console.error('Invalid AI response format:', recipes)
      return NextResponse.json({ error: 'Invalid AI response format', aiResponse: recipesString }, { status: 500 })
    }

    // Store the recipes in the database
    const storedRecipes = await Promise.all(recipes.map(async (recipe: any) => {
    ...
    return NextResponse.json({ error: 'Failed to find recipes', details: (error as Error).message }, { status: 500 })
  }
}